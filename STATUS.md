...
1. 阶段：M3 本地通知闭环
2. 状态：已完成通知权限页、通知服务抽象、创建事项后的通知调度入口，并规避 Expo Go Android 启动期红色警告
...
6. `expo-notifications` 已接入服务层和页面入口；Expo Go Android 下不主动加载原生通知模块，完整本地通知验证需要 development build
7. 当前尚未接入广告、会员支付和云同步
